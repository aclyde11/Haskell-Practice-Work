...
studentsOfTeacher_ students teacher =
   map listStudentsInCourse (courses_teaching teacher) where
   listStudentsInCourse c =
      let studentsInCourse = filter (elem (dept teacher, c) . courses_enrolled) students
      in (c, map (\s->(lastName s, firstName s)) studentsInCourse)
